...
class MultiDroneCoverageEnv(gym.Env):
    ...
    # ----------------------------------------------------------------------
    def get_lidar_scan(self, drone_idx: int) -> np.ndarray:
        """
        Simulate a 2D horizontal LiDAR scan around the drone.
        Returns 36 range measurements (every 10 degrees) up to 10.0m max range.
        Fuses static and dynamic obstacle proximity.
        """
        pos = self.positions[drone_idx]
        ranges = np.full(36, 10.0, dtype=np.float32)
        
        # Check static obstacles in 3D grid
        idx_s = np.argwhere(self.static)
        if idx_s.size > 0:
            centers = idx_s.astype(float) + 0.5 # (M, 3)
            # Filter obstacles close in Z to drone altitude
            z_diff = np.abs(centers[:, 2] - pos[2])
            close_mask = z_diff < 1.0
            if np.any(close_mask):
                close_centers = centers[close_mask]
                diffs = close_centers[:, :2] - pos[:2]
                dists = np.linalg.norm(diffs, axis=1)
                obs_angles = np.arctan2(diffs[:, 1], diffs[:, 0]) % (2 * np.pi)
                for dist, obs_ang in zip(dists, obs_angles):
                    if dist < 10.0:
                        bin_idx = int(round(obs_ang / (2 * np.pi) * 36)) % 36
                        ranges[bin_idx] = min(ranges[bin_idx], dist)
                        
        # Check dynamic obstacles
        if hasattr(self.dyn, "positions") and self.dyn.positions:
            dpos = np.array(self.dyn.positions) # (K, 3)
            z_diff = np.abs(dpos[:, 2] - pos[2])
            close_mask = z_diff < 1.0
            if np.any(close_mask):
                close_dpos = dpos[close_mask]
                diffs = close_dpos[:, :2] - pos[:2]
                dists = np.linalg.norm(diffs, axis=1)
                obs_angles = np.arctan2(diffs[:, 1], diffs[:, 0]) % (2 * np.pi)
                for dist, obs_ang in zip(dists, obs_angles):
                    if dist < 10.0:
        ...
        # Add Gaussian noise
        noise = np.random.normal(0.0, 0.05, size=36).astype(np.float32)
        ranges = np.clip(ranges + noise, 0.1, 10.0)
        return ranges

    def get_camera_image(self, drone_idx: int) -> np.ndarray:
        """
        Simulate a downward-facing camera detecting ground terrain features,
        recharge stations, and local coverage percentage.
        Returns a 10-dimensional flat feature vector.
        """
        pos = self.positions[drone_idx]
        xi = int(np.clip(pos[0], 0, self.GX - 1))
        yi = int(np.clip(pos[1], 0, self.GY - 1))
        ...
        terrain_height = float(self.terrain[xi, yi])
        # simple finite differences for gradient
        dx = float(self.terrain[min(xi+1, self.GX-1), yi] - self.terrain[max(xi-1, 0), yi]) / 2.0
        dy = float(self.terrain[xi, min(yi+1, self.GY-1)] - self.terrain[xi, max(yi-1, 0)]) / 2.0
        
        # Coverage fraction in 5x5 window
        x0, x1 = max(0, xi - 2), min(self.GX, xi + 3)
        y0, y1 = max(0, yi - 2), min(self.GY, yi + 3)
        window = self.coverage_grid[x0:x1, y0:y1]
        coverage_frac = float(np.sum(window)) / max(1.0, float(window.size))
        
        # Relative displacement to nearest recharge station
        nearest_rc = self._nearest_recharge(pos)
        rc_rel = nearest_rc - pos
        
        # Displacement to nearest dynamic obstacle if within 8.0m horizontal range
        obs_rel = np.zeros(3, dtype=np.float32)
        if hasattr(self.dyn, "positions") and self.dyn.positions:
            dpos = np.array(self.dyn.positions)
            diffs = dpos - pos.reshape(1, 3)
            dists = np.linalg.norm(diffs[:, :2], axis=1)
            closest_idx = np.argmin(dists)
            if dists[closest_idx] < 8.0:
                obs_rel = diffs[closest_idx].astype(np.float32)

        features = np.array([
            terrain_height, dx, dy, coverage_frac,
            rc_rel[0], rc_rel[1], rc_rel[2],
            obs_rel[0], obs_rel[1], obs_rel[2]
        ], dtype=np.float32)
        
        # Add tiny sensor noise
        features += np.random.normal(0.0, 0.02, size=features.shape)
        return features

    def get_ultrasonic_range(self, drone_idx: int) -> float:
        """
        Simulate a downward ultrasonic range sensor measuring altitude AGL.
        """
        pos = self.positions[drone_idx]
        xi = int(np.clip(pos[0], 0, self.GX - 1))
        yi = int(np.clip(pos[1], 0, self.GY - 1))
        terrain_height = float(self.terrain[xi, yi])
        height_agl = max(0.0, pos[2] - terrain_height)
        
        # Add noise and clip
        noise = float(np.random.normal(0.0, 0.02))
        return float(np.clip(height_agl + noise, 0.1, 10.0))

    def get_imu_data(self, drone_idx: int) -> dict:
        """
        Simulate IMU measurements: linear acceleration and angular velocity.
        """
        vel = self.velocities[drone_idx]
        # accel includes gravity
        wind = self.wind_field.at(self.positions[drone_idx]) if self.wind_enabled else np.zeros(3)
        accel = (vel * 0.1) + np.random.normal(0.0, 0.05, size=3)
        accel[2] += 9.81  # IMU measures acceleration relative to freefall
        
        gyro = np.random.normal(0.0, 0.02, size=3)
        
        yaw = float(np.arctan2(vel[1], vel[0])) if np.linalg.norm(vel[:2]) > 0.1 else 0.0
        pitch = float(np.clip(-vel[2] / self.max_speed * 0.2, -0.3, 0.3))
        roll = 0.0
