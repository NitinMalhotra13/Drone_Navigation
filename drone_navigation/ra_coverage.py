...
class RavenReplanner:
    ...
    def _init_population(
        ...
        for _ in range(self.n_ravens):
            ...
            for d in range(self.n_drones):
                if self.n_drones == 3:
                    # Offset return sweeps to match the forward sweep sides and prevent path crossing:
                    # Drone 0 -> bottom-right gap (18.0)
                    # Drone 1 -> top-left gap (-18.0)
                    # Drone 2 -> top-left border (-40.0)
                    corridor_centers = [18.0, -18.0, -40.0]
                    corridor_center = corridor_centers[d]
                elif self.n_drones == 6:
                    # Offset return sweeps into the gaps between forward sweeps to cover new ground:
                    # Drone 0 (bottom-right) -> gap 1 (36.0)
                    # Drone 1 (bottom-right) -> gap 2 (18.0)
                    # Drone 2 (bottom-right) -> gap 3 (4.5)
                    # Drone 3 (top-left)     -> gap 4 (-4.5)
                    # Drone 4 (top-left)     -> gap 5 (-18.0)
                    # Drone 5 (top-left)     -> gap 6 (-36.0)
                    corridor_centers = [36.0, 18.0, 4.5, -4.5, -18.0, -36.0]
                    corridor_center = corridor_centers[d]
                else:
                    d_inv = self.n_drones - 1 - d
                    corridor_center = -40.0 + (80.0 * d_inv / max(1, self.n_drones - 1))
